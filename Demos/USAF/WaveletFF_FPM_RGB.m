...
figure;
[temp,rect] = imcrop((imread([path{1},num2str(113),'.tif'])));
...
[wavefront1,...
 wavefront2,...
 fpm_cube,pratio,Pupil0,kc] = utils.perpare_trainingdata(path,...
                                                    rect,...
                                                    color_index,...
                                                    pix,...
                                                    led_num,...
...
while epoch < numEpochs
    ...
    all_images = led_num^2;

    while fpm_cube.hasdata()
        all_images = max(all_images - batch_size,0);
        clc
        disp("processing bacth, image remains ... " + all_images + "/" + led_num^2);
        ...
        % TV-reg
        dldw1 = dldw1 + 0.1*tv_reg(wavefront1);
    ...
    if mod(epoch,1) == 0
        o = wavefront1;
        % wavefront1 = fft2_ware(sign(o),true);

        F = utils.fft2_ware(wavefront1,true);
